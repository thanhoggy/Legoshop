                            ...
                            <tr>
                                <td class="cart_product">
                                    @foreach($image_products as $image_product)
                                        <a href=""><img src="{{url('products/small',$image_product->image)}}" alt="" style="width: 100px;padding:10px;"></a>
                                    @endforeach
                                </td>
                    ...
                    <div class="chose_area" style="padding: 20px;">
                        <form action="{{url('/apply-coupon')}}" method="post" role="form">
                            ...
                            <div class="form-group">
                                <label for="coupon_code">Mã KM</label>
                                <div class="controls {{$errors->has('coupon_code')?'has-error':''}}">
                                    <input type="text" class="form-control" name="coupon_code" id="coupon_code" placeholder="Nhập mã ở đây">
                    ...
                    <div class="total_area">
                        <ul>
                            @if(Session::has('discount_amount_price'))
                                <li>Tạm tính <span>{{$total_price}} VND </span></li>
                                <li>Giảm giá (Mã KM : {{Session::get('coupon_code')}}) <span>$ {{Session::get('discount_amount_price')}}</span></li>
                                <li>Tổng cộng <span>{{$total_price-Session::get('discount_amount_price')}} VND </span></li>
                            @else
                                <li>Tổng cộng <span>{{$total_price}} VND </span></li>
                            @endif
                        </ul>
                        <div style="margin-left: 20px;"><a class="btn btn-default check_out" href="{{url('/check-out')}}">Thanh toán</a></div>
                    </div>
                </div>
            </div>
        </div>
